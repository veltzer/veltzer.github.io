...
- ~~**The About page's contact line points at Gitter.** — DONE.~~
  `profiles.yaml`'s `contact` entry linked `gitter.im/veltzer/mark.veltzer`, which 301s
  into Matrix and is effectively dead. The entry is now a plain link (`link_text` +
  `url`) carrying the email, and both renderers (`scripts/gen_profiles.py` here,
  `scripts/gen_readme.py` in `../veltzer`) were changed to match, so the About page and
  the GitHub profile README say the same thing. Gitter was also removed from the rest of
  the fleet at the same time: the shared `tera.templates/README.md.tera` carried a
  gitter badge in its "contact me" section in 107 repos, and a few hand-written READMEs
  and old mako templates had a "chat with me at" line; all gone, the READMEs
  regenerated by each repo's own build.

- ~~**`/blog/` is a bare 404.**~~ Fixed 2026-09-22. The nine section roots
  (`/blog/`, `/tags/`, `/about/`, `/media/`, `/chess/`, `/slides/`, `/syllabi/`,
  `/animations/`, `/training/`) are in `LEGACY_REDIRECTS` alongside the post URLs.
  Done as part of the Search Console round that failed validation on 9/15 -- the
  section roots were the same defect as the post URLs and would have failed the
  next validation the same way. See `doc/SEO.md`.
